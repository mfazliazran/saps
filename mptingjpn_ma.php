...
$res=oci_parse($conn_sispa,"select negeri,negeri from tknegeri where kodnegeri='$kodnegeri'");
oci_execute($res);
...
echo "<h3><center>$namappd<br>ANALISA MATA PELAJARAN TINGKATAN $ting<br>".jpep("".$_SESSION['jpep']."")." TAHUN $tahun</center></h3>";
...
$q_mp = "SELECT mp.mp,amp.KODMP, SUM(bcalon) bcalon, SUM(ambil) ambil, SUM(TH) th, SUM(Ap) bilap, SUM(A) bila, SUM(Am) bilam, 
					 ...
					 AND  amp.ting='$ting' AND amp.kodmp=mp.kod and ts.kodnegerijpn='$kodnegeri' Group BY mp.mp,amp.KODMP ORDER BY mp.mp ";
...
?>
<br><br>
&nbsp;&nbsp;<input type="button" name="cetak" value="CETAK" onClick="document.f1.action='ctk_mptingmajpn.php?<?php echo "ting=$ting&tahun=$tahun&jpep=$jpep";?>';document.f1.submit();">
<input type="button" name="export" value="EXPORT KE EXCEL" onclick="document.f1.action='mptingmajpn_excel.php?<?php echo "ting=$ting&tahun=$tahun&jpep=$jpep";?>';document.f1.submit();" />
</form> 
