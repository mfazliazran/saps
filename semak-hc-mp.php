...
$q_kelas = oci_parse($conn_sispa,"SELECT * FROM tkelassek WHERE tahun='".$_SESSION['tahun']."' AND kodsek='".$_SESSION['kodsek']."' AND ting='$ting' ORDER BY kelas");
oci_execute($q_kelas);
while ($rowkelas = oci_fetch_array($q_kelas))
{
	$kelas = $rowkelas['KELAS'];
	$qhcmurid = "SELECT * FROM $theadcount WHERE tahun='$tahun' AND kodsek='$kodsek' AND $tahap='$ting' AND kelas='$kelas' AND hmp='$kod' and gtov is not null";
	//if($kodsek=='AEE3045')
		//echo $qhcmurid."<br>";
