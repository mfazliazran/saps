...
include 'fungsi2.php';
  ...
  if ($_SESSION["level"]<>"6")
    die ("Skrin ini hanya untuk level JPN");
  ...
  $negeri=$data2["NEGERI"];
	...
	if (count_row($qry_su)>0){
		     message("Username telah wujud..sila tukar yang lain",1);
     		 location("tambah-user-jpn.php");
	} else {							

	  $query_sm=" insert into login(tahun,nokp,user1,pswd,kodsek,namasek,nama,level1,statussek,negeri,daerah,kodnegeri) 
				  values ('$tahun','$nokp','$login','$katalaluan','$kodjpn','$jawatan','$nama','$level','$statussek','$negeri','','$kodjpn')";
...
<tr><td>LEVEL</td><td>:</td><td><b>JPN</b><input type="hidden" name="txtLevel" value="6"></td></tr>
