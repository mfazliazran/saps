...
set_time_limit(0);
$darjah = $_GET["tahap"];
$namkelas = oci_escape_string($_GET["namakelas"]);
...
if(date('Y')=='2019')
	...
	$sk = "select TO_CHAR(tkhmigrate,'DD/MM/YYYY HH:MI:SS') as TARIKH_SYNC from $table where KODSEK$darjah='$kodsek' and $darjah=$darjah and TAHUN$darjah='".$_SESSION['tahun']."' and KELAS$darjah='$namkelas' order by tkhmigrate desc";
...
<table width="578" border="1" align="center">
  <tr bgcolor="#ff9900">
    <td align="center" bgcolor="#ff9900">Proses import data murid terdahulu telah dilakukan pada <strong><?php echo $tarikhkemas; ?></strong></td>
  </tr>
  <tr bgcolor="#ff9900">
    <td align="center"><input type="submit" name="button2" id="button2" value="IMPORT DATA <?php echo "$jenis $darjah $namkelas";?> DARI APDM" onclick="return semak_input();"/>
...
if($_POST['post']=='1'){
	...
	$sql="SELECT NAMA,NOKP,NOSIJILLAHIR,KODKAUM,KODAGAMA,KODJANTINA,KODTINGKATANTAHUN,NAMAKELAS,NOKPGURU,NAMAGURU,KODSEKOLAH,NAMASEKOLAH,IDKODJENISSEKOLAH,IDKODPPD,IDKODNEGERI FROM DATA_SEMUA_MURID WHERE KODSEKOLAH='$kodsekolah' AND TRIM(KODTINGKATANTAHUN)='$darjah' and NAMAKELAS='$namkelas' ORDER BY TRIM(KODTINGKATANTAHUN),NAMA";
...
//die($sql);
	...
	$masa = date("d/m/Y H:i:s");
  	...
  	while($dt=oci_fetch_array($stmt)){
		...
		if($bil==1 or ($semakting <> $KODTINGKATANTAHUN)){
			...
			$sqlup=oci_parse($conn_sispa,"update $table set KODSEK$KODTINGKATANTAHUN=null,TAHUN$KODTINGKATANTAHUN=null,$KODTINGKATANTAHUN=null,KELAS$KODTINGKATANTAHUN=null where KODSEK$KODTINGKATANTAHUN='$KODSEK' AND TAHUN$KODTINGKATANTAHUN='".$_SESSION['tahun']."' AND $KODTINGKATANTAHUN='$KODTINGKATANTAHUN' AND KELAS$KODTINGKATANTAHUN='$NAMAKELAS'");
			oci_execute($sqlup);
		}
		...
		if($dt2=oci_fetch_array($stmt2)){
			...
			if($dt3=oci_fetch_array($stmt3)){
				...
				$ins++;
					...
					echo "<br><b>$NAMA [$KODTINGKATANTAHUN - $NAMAKELAS]</b> tidak mempunyai No. Kad Pengenalan / No. Sijil Lahir. Sistem SAPS tidak dapat menyimpan data tersebut.<br>";
				}
			}
			//insert new student
			/*$cnt++;
			$insert = "insert into $table (NOKP,NAMAP,KODSEK$KODTINGKATANTAHUN,TAHUN$KODTINGKATANTAHUN,$KODTINGKATANTAHUN,KELAS$KODTINGKATANTAHUN,JANTINA,KAUM,AGAMA,TKHMIGRATE)values('$nokputama','$NAMA','$KODSEK','".$_SESSION['tahun']."','$KODTINGKATANTAHUN','$NAMAKELAS','$KODJANTINA','$KODKAUM','$KODAGAMA',to_date('$masa','dd/mm/yyyy hh24:mi:ss'))";
			$sql_insrt=oci_parse($conn_sispa,$insert);
			oci_execute($sql_insrt);
			//echo $insert."<br>";
			$ins++;*/
	...
	echo "<br>Proses import data murid dari APDM selesai.....<br>";
