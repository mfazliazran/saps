...
/*
|---------------------------------------|
|  DAPATKAN KOD SUBJEK YG DIDAFTARKAN   |
|---------------------------------------|
*/
...
while ($recordting = oci_fetch_array($rting)){
	...
	$qhc = "SELECT * FROM $theadcount WHERE kodsek='$kodsek' AND tahun='$tahun' AND hmp='$hmp' AND $tahap='$ting' and gtov is not null";
	...
	switch ($ting) {
		case "P": case "p": case "T1": case "t1": case "T2": case "t2": case "T3": case "t3": case "D1": case "d1": case "D2": case "d2": case "D3": case "d3": case "D4": case "d4": case "D5": case "d5": case "D6": case "d6":
		{
			...
			switch ($level)
			{
				...
				case "SR" :
				{
					if($tahun<=2014){
						...
						//echo $gpmpetr." - ".$etrA." - ".$etrB." - ".$etrC." - ".$etrD." - ".$etrE." - ".$etrTH."<br>";	
					}
					break;	
				}
			}
			...
			echo "<td><div align=\"left\"><a href=hc-mp-kelas.php?datahc=".$ting."|".$hmp.">$tempmp</div></td>\n";
		...
		{	
			...
			$peratusoti1lulus = number_format(($oti1lulus/($bilcalon-$oti1TH))*100,2,'.',',');
			//echo $peratusoti1lulus." - ($oti1lulus/($bilcalon-$oti1TH)) <br>";
			$peratusoti2lulus = number_format(($oti2lulus/($bilcalon-$oti2TH))*100,2,'.',',');
			$peratusoti3lulus = number_format(($oti3lulus/($bilcalon-$oti3TH))*100,2,'.',',');
...
<br />
<?php
echo "<table width=\"80%\"  border=\"1\" align=\"left\" cellpadding=\"5\" cellspacing=\"0\" bordercolor=\"#999999\">";
echo "<tr><td ><div align=\"center\">&nbsp;</div></td>\n";
echo "<td><div align=\"center\">Nota Pengiraan GPMP</div></td></tr>\n"; 
echo "<tr><td ><div align=\"center\">SR/MR</div></td>\n";
echo "<td><div align=\"left\">GPMP = ((bilA*1) + (bilB*2) + (bilC*3) + (bilD*4) + (bilE*5))/ambil</div></td></tr>\n"; 
echo "<tr><td ><div align=\"center\">MA</div></td>\n";
echo "<td><div align=\"left\">GPMP = ((bilA+*0) + (bilA*1) + (bilA-*2) + (bilB+*3) + (bilB*4) + (bilC+*5) + (bilC*6) + (bilD*7) + (bilE*8) + (bilG*9))/ambil</div></td></tr>\n"; 
echo "</table>\n";
?>
</tr></table>
</td>
