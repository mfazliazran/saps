...
echo "<h3><center>$namasek<br>ANALISA MATA PELAJARAN TINGKATAN $ting<br>".("".$_SESSION['jpep']."")." TAHUN ".$_SESSION['tahun']."</center></h3>";
...
$q_mp = "SELECT amp.kodmp, mp.mp, SUM(bcalon) bcalon, SUM(ambil) ambil, SUM(TH) th, SUM(Ap) bilap, SUM(A) bila, SUM(Am) bilam, SUM(Bp) bilbp, SUM(B) bilb, 
SUM(Cp) bilcp, SUM(C) bilc, SUM(D) bild, SUM(E) bile, SUM(G) bilg FROM analisis_mpma amp, mpsmkc mp 
WHERE amp.kodsek=:kodsek and amp.tahun=:tahun AND amp.jpep=:jpep AND  amp.ting=:ting AND amp.kodmp=mp.kod 
Group BY amp.kodmp, mp.susun, mp.mp ORDER BY mp.susun";
...
$bilmp = kira_bil_rekod("SELECT mp.mp, SUM(bcalon) bcalon, SUM(ambil) ambil, SUM(TH) th, SUM(Ap) bilap, SUM(A) bila, SUM(Am) bilam, 
					 ...
					 AND  amp.ting=:ting AND amp.kodmp=mp.kod Group BY mp.mp ORDER BY mp.mp ",$prm,$val);
...
$q_mp = "SELECT amp.kodmp, mp.mp, SUM(bcalon) bcalon, SUM(ambil) ambil, SUM(TH) th, SUM(Ap) bilap, SUM(A) bila, SUM(Am) bilam, SUM(Bp) bilbp, 
        SUM(B) bilb, SUM(Cp) bilcp, SUM(C) bilc, SUM(D) bild, SUM(E) bile, SUM(G) bilg FROM analisis_mpma amp, mpsmkc mp 
		WHERE amp.kodsek=:kodsek and amp.tahun=:tahun AND amp.jpep=:jpep AND  amp.ting=:ting AND amp.kodmp=mp.kod 
		Group BY amp.kodmp, mp.susun, mp.mp ORDER BY mp.susun";
$qry = oci_parse($conn_sispa,$q_mp);
oci_bind_by_name($qry,":kodsek",$kodsek);
