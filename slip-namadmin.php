...
{		
	...
	$q_guru = oci_parse($conn_sispa,"SELECT * FROM tguru_kelas gk, tsekolah ts WHERE gk.kodsek='".$_SESSION['kodsek']."' AND gk.tahun='".$_SESSION['tahun']."' AND gk.ting='$ting' AND gk.kelas='$kelas' AND gk.kodsek=ts.kodsek");
	oci_execute($q_guru);
	...
	switch ($ting)
	{
		case "D1": case "D2" : case "D3": case "D4" :case "D5" : case "D6" :
			location("slip_namadminsr.php?ting=$ting&kelas=".urlencode($kelas)."&&namaguru=$namagu&&tingkatan=$tingkatan&&namasekolah=$namasek");
			break;
		case "P": case "T1": case "T2": case "T3":
			location("slip_namadminmr.php?ting=$ting&kelas=".urlencode($kelas)."");//&&namaguru=$namagu&&tingkatan=$tingkatan&&namasekolah=$namasek");
			break;
		case "T4": case "T5":
			location("slip_namadminma.php?ting=$ting&kelas=".urlencode($kelas)."");//&&namaguru=$namagu&&tingkatan=$tingkatan&&namasekolah=$namasek");
			break;
	}
}
else{
?>
		...
		self.location='slip-namadmin.php?ting=' + val;
		}
		</script>
		<?php
		...
		if($ting<>""){
		$kelas_sql = oci_parse($conn_sispa,"SELECT DISTINCT kelas FROM tkelassek WHERE tahun ='".$_SESSION['tahun']."' AND kodsek='".$_SESSION['kodsek']."' AND ting='$ting' ORDER BY kelas");
		oci_execute($kelas_sql);
		}
		echo "<form method=post name='f1' action='slip-namaadmin.php'>";
