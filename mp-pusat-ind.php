...
$level=$_SESSION['level'];
  $m=$_GET['data'];
  list( $kodnegeri, $ting, $kodmp, $tahun, $jpep, $status)=split('[/]', $m);
...
$rmp=oci_fetch_array($qmp);
  ...
  $namanegeri1=oci_parse($conn_sispa,"select negeri from tknegeri where kodnegeri='$kodnegeri'");
	...
	?>
   <?php
   echo "<center><h3>ANALISIS PENCAPAIAN <br>MATA PELAJARAN ".$rmp["MP"]." <br>".jpep($jpep)." ".tahap($ting)." <br>NEGERI $negeri TAHUN $tahun </h3></center><br>";
...
echo "  <tr>\n";
	...
	if ($level=='8')
		$q_mp = "SELECT  analisis_mpma.kodppd, SUM(bcalon) as bcalon, SUM(ambil) ambil, SUM(TH) th, SUM(Ap) bilap, SUM(A) bila, SUM(Am) bilam, SUM(Bp) bilbp, SUM(B) bilb, SUM(Cp) bilcp, SUM(C) bilc, SUM(D) bild, SUM(E) bile, SUM(G) bilg 
		FROM analisis_mpma,tsekolah WHERE tahun='$tahun' AND analisis_mpma.negeri='$negeri' AND jpep='$jpep' AND  ting='$ting' AND kodmp='$kodmp' 
		and analisis_mpma.kodsek=tsekolah.kodsek and tsekolah.kodjenissekolah in ('202','203') 
		Group BY analisis_mpma.kodppd ORDER BY analisis_mpma.kodppd ";
	else
		$q_mp = "SELECT  kodppd, SUM(bcalon) as bcalon, SUM(ambil) ambil, SUM(TH) th, SUM(Ap) bilap, SUM(A) bila, SUM(Am) bilam, SUM(Bp) bilbp, SUM(B) bilb, SUM(Cp) bilcp, SUM(C) bilc, SUM(D) bild, SUM(E) bile, SUM(G) bilg 
		FROM analisis_mpma WHERE tahun='$tahun' AND negeri='$negeri' AND jpep='$jpep' AND  ting='$ting' AND kodmp='$kodmp' 
		Group BY kodppd ORDER BY kodppd ";		
	$qry = oci_parse($conn_sispa,$q_mp);
	oci_execute($qry);
	
	//echo"Level: $level<br>SQL: $q_mp<br>";
	...
	echo "    <td><center><a href=mp-jpn-ind.php?data=".$paparppd."/".$ting."/".$kodmp."/".$tahun."/".$jpep."/".$status."  target=_blank>$paparppd - $namappd</a></center></span></td>\n";
	...
	if (($ting=="T1") OR ($ting=="T2") OR ($ting=="T3")){
		...
		echo "  <tr>\n";
			...
			$bil=0;
		if ($level=='8')
			$q_mp = "SELECT  analisis_mpmr.kodppd, SUM(bcalon) bcalon, SUM(ambil) ambil, SUM(TH) th, SUM(A) bila, SUM(B) bilb, SUM(C) bilc, SUM(D) bild, SUM(E) bile 
			FROM analisis_mpmr,tsekolah WHERE tahun='$tahun' AND analisis_mpmr.negeri='$negeri' AND jpep='$jpep' AND  ting='$ting' AND kodmp='$kodmp' 
			analisis_mpmr.kodsek=tsekolah.kodsek and tsekolah.kodjenissekolah in ('202','203') 
			Group BY analisis_mpmr.kodppd ORDER BY analisis_mpmr.kodppd ";
		else	
			$q_mp = "SELECT  kodppd, SUM(bcalon) bcalon, SUM(ambil) ambil, SUM(TH) th, SUM(A) bila, SUM(B) bilb, SUM(C) bilc, SUM(D) bild, SUM(E) bile 
			FROM analisis_mpmr WHERE tahun='$tahun' AND negeri='$negeri' AND jpep='$jpep' AND  ting='$ting' AND kodmp='$kodmp' 
			Group BY kodppd ORDER BY kodppd ";
		$qry = oci_parse($conn_sispa,$q_mp);
		oci_execute($qry);
		//echo"mr--$q_mp<br>";
...
if (($ting=="D1") OR ($ting=="D2") OR ($ting=="D3") OR ($ting=="D4") OR ($ting=="D5") OR ($ting=="D6")){
	...
	$bil=0;
	if ($level=='8')
		$q_mp = "SELECT  analisis_mpsr.kodppd, SUM(bcalon) bcalon, SUM(ambil) ambil, SUM(TH) th, SUM(A) bila, SUM(B) bilb, SUM(C) bilc, SUM(D) bild, SUM(E) bile, SUM(F) bilf 
		FROM analisis_mpsr WHERE tahun='$tahun' AND analisis_mpsr.negeri='$negeri' AND jpep='$jpep' AND  darjah='$ting' AND kodmp='$kodmp' 
		analisis_mpsr.kodsek=tsekolah.kodsek and tsekolah.kodjenissekolah in ('202','203') 
		Group BY analisis_mpsr.kodppd ORDER BY analisis_mpsr.kodppd ";
	else
		$q_mp = "SELECT  kodppd, SUM(bcalon) bcalon, SUM(ambil) ambil, SUM(TH) th, SUM(A) bila, SUM(B) bilb, SUM(C) bilc, SUM(D) bild, SUM(E) bile, SUM(F) bilf 
		FROM analisis_mpsr WHERE tahun='$tahun' AND negeri='$negeri' AND jpep='$jpep' AND  darjah='$ting' AND kodmp='$kodmp' 	
		Group BY kodppd ORDER BY kodppd ";
