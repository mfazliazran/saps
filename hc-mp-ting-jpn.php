...
<?php
$datahc = $_GET['datahc'];
//list ($kodsek, $tahun ,$ting)=split('[/]', $datahc); 
list ($ting)=split('[/]', $datahc); 
...
//if($kodsek=='BEA8638')
	//	echo "SELECT * FROM tentu_hc WHERE tingpep='$ting' AND tahunpep='".$_SESSION['tahun']."' ORDER BY capai<br>";
//echo SELECT * "FROM tentu_hc WHERE tingpep='$tingtemp' AND tahunpep='".$_SESSION['tahun']."' ORDER BY capai";
oci_execute($qryatr);
$bil=0;
...
echo "    <td rowspan=\"2\">Mata Pelajaran\n";
echo "    <div align=\"center\"></div></td>\n";
...
while (OCIFetch($rting)){
		$ting = OCIResult($rting,"$tahap");//$recordting["$tahap"];
		echo "<br>".$ting;
		
}*/

$qting = "SELECT DISTINCT hmp FROM $theadcount WHERE kodsek='$kodsek' AND tahun='$tahun' AND $tingdar='$ting' ORDER BY hmp";
//echo "SELECT DISTINCT hmp FROM $theadcount WHERE tahun='$tahun' AND kodsek='$kodsek' AND $tingdar='$ting' ORDER BY hmp";
...
while ($recordting = oci_fetch_array($rting)){
	$hmp = $recordting['HMP'];
	$gmp = "G$hmp";
	$qmp = oci_parse($conn_sispa,"SELECT * FROM $tmatap WHERE kod='$hmp'");
	oci_execute($qmp);
	$resultmp = oci_fetch_array($qmp);
	$tempmp = $resultmp['MP'];

	$q_atr1 = "SELECT $gmp FROM $tmarkah WHERE kodsek='$kodsek' AND tahun='$tahunatr1' AND $tahap='$tingatr1' AND jpep='$jpepatr1' AND $gmp IS NOT NULL";
	$qry_atr1 = oci_parse($conn_sispa,$q_atr1);
	oci_execute($qry_atr1);
	$calon_atr1 = count_row($q_atr1);
	//if($kodsek=='BEA8638')
		//echo $calon_atr1."<br>";
	$q_atr2 = "SELECT $gmp FROM $tmarkah WHERE kodsek='$kodsek' AND tahun='$tahunatr2' AND $tahap='$tingatr2' AND jpep='$jpepatr2' AND $gmp IS NOT NULL";
	$qry_atr2 = oci_parse($conn_sispa,$q_atr2);
	oci_execute($qry_atr2);
	$calon_atr2 = count_row($q_atr2);
	$q_atr3 ="SELECT $gmp FROM $tmarkah WHERE kodsek='$kodsek' AND tahun='$tahunatr3' AND $tahap='$tingatr3' AND jpep='$jpepatr3' AND $gmp IS NOT NULL";
	$qry_atr3 = oci_parse($conn_sispa,$q_atr3);
	oci_execute($qry_atr3);
	$calon_atr3 = count_row($q_atr3);
	
	$qhc = "SELECT * FROM $theadcount WHERE kodsek='$kodsek' AND tahun='$tahun' AND hmp='$hmp' AND $tahap='$ting'";
	$bilcal = "SELECT * FROM $theadcount WHERE kodsek='$kodsek' AND tahun='$tahun' AND hmp='$hmp' AND $tahap='$ting' and gtov is not null";
	//if($kodsek=='AEA2053')
		//echo $qhc."<br>";
		...
		case "P": case "p": case "T1": case "t1": case "T2": case "t2": case "T3": case "t3": case "D1": case "d1": case "D2": case "d2": case "D3": case "d3": case "D4": case "d4": case "D5": case "d5": case "D6": case "d6":
		{
			...
			if($bil&1) {
				...
				while($data_hc = oci_fetch_array($res_hc)){
					...
					//$q_mp = "SELECT amp.kodmp, SUM(bcalon) bcalon, SUM(ambil) ambil, SUM(TH) th, SUM(A) bila, SUM(B) bilb, SUM(C) bilc, SUM(D) bild, SUM(E) bile FROM $analisis WHERE amp.kodsek='$kodsek' and amp.tahun='$tahun' AND amp.jpep='$jenpep' AND  amp.$tingdar='$ting' AND amp.kodmp=mp.kod and amp.kodmp='$hmp' and amp.kodmp not in ($minus) Group BY mp.susun,amp.kodmp";
					$q_mp = "SELECT amp.kodmp, SUM(bcalon) bcalon, SUM(ambil) ambil, SUM(TH) th, SUM(A) bila, SUM(B) bilb, SUM(C) bilc, SUM(D) bild, SUM(E) bile FROM $analisis WHERE amp.kodsek='$kodsek' and amp.tahun='$tahun' AND amp.jpep='$jenpep' AND  amp.$tingdar='$ting' AND amp.kodmp=mp.kod and amp.kodmp='$hmp' Group BY mp.susun,amp.kodmp";
					//if($kodsek=='BBA4042')
						//echo "$q_mp<br>";
			...
			echo "<td><div align=\"left\"><a href=hc-mp-kelas-jpn.php?datahc=".$ting."|".$hmp.">$tempmp</div></td>\n";
			...
			$gpmpetr = number_format((($etr1AA*0)+($etr1A*1)+($etr2A*2)+($etr3B*3)+($etr4B*4)+($etr5C*5)+($etr6C*6)+($etr7D*7)+($etr8E*8)+($etr9G*9))/($bilcalon-$etrTH),2,'.',',');
				...
				while($data_ha = oci_fetch_array($res_ha)){
					...
					//echo "<br>".$capai2;
					 ...
					 AND  amp.$tingdar='$ting' AND amp.kodmp=mp.kod and amp.kodmp='$hmp' and amp.kodmp not in ($minus) Group BY mp.susun,amp.kodmp";
