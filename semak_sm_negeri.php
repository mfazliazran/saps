...
<p class="subHeader">Senarai Semak Sekolah Menengah <font color="#FFFFFF">(Tarikh Kemaskini Program : 10/8/2011 11.20AM)</font></p>
<?php
//$nokp="".$_SESSION['nokp']."";
...
if ($_SESSION['level']=="6")
	$querysub = "SELECT KODSEK,NAMASEK FROM tsekolah WHERE STATUS='SM' and KODNEGERIJPN='".$_SESSION["kodnegeri"]."' order by KODSEK ASC, NAMASEK ASC";//DONE INDEXING
elseif ($_SESSION['level']=="7")
	$querysub = "SELECT KODSEK,NAMASEK FROM tsekolah WHERE STATUS='SM' order by KODSEK ASC, NAMASEK ASC";//DONE INDEXING
...
if($jenispep<>""){
	...
	while(OCIFetch($qry)) {
		...
		$sql="select count(nokp) as bilmurid from tmurid where (KodSekT4='$kodsekolah' and TahunT4='$tahun') or (KodSekT5='$kodsekolah' and TahunT5='$tahun')";
		$res=oci_parse($conn_sispa,$sql);	 
		oci_execute($res);
		$data=oci_fetch_array($res);
		$cnt_murid=$data["BILMURID"];
		
		//P, T1,T2,T3 (DISABLED 01/10/2014)
		/*
		$sql5 = "select ting,kodmp,sum(bilammp) as BILREKOD from sub_guru where kodsek='$kodsekolah' AND tahun='$tahun' AND KODMP IN (SELECT KOD FROM SUB_MR) group by ting,kodmp";//DONE INDEXING
		//echo $sql5;
		$res5=oci_parse($conn_sispa,$sql5);
		oci_execute($res5);
		if(oci_error($res5)){
			$qryerr = "select ting,kodmp,kelas,bilammp,nama,nokp from sub_guru where kodsek='$kodsekolah' AND tahun='$tahun' AND (bilammp like '% %' or bilammp like '%O%') AND KODMP IN (SELECT KOD FROM SUB_MR)";
		...
		*/
		...
		$sql7 = "select ting,kodmp,sum(bilammp) as BILREKOD from sub_guru where kodsek='$kodsekolah' AND tahun='$tahun' AND kodmp NOT IN (select KOD from SUB_MA_XAMBIL) group by ting,kodmp";//DONE INDEXING
		//echo $sql7;
		$res7=oci_parse($conn_sispa,$sql7);
		oci_execute($res7);
		if(oci_error($res7)){
			//echo $sql7."<br>";
			$qryerr7 = "select ting,kodmp,kelas,bilammp,nama,nokp from sub_guru where kodsek='$kodsekolah' AND tahun='$tahun' AND (bilammp like '% %' or bilammp like '%O%' or bilammp like '%`%') AND kodmp NOT in (select KOD from SUB_MA_XAMBIL)";
			//echo $qryerr7."<br>";
