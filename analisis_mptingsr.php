...
<form action="ctk_analisis-mptingsr.php?ting=<?php echo $m;?>" method="POST" target="_blank">

<td valign="top" class="rightColumn">
<p class="subHeader">Analisa Mata Pelajaran Tingkatan / Tahun<font color="#FFFFFF"> (Tarikh Kemaskini 19/3/2015) </font></p>
<?php
echo "<h5><center>$namasek<br>ANALISA MATA PELAJARAN TAHUN $ting<br>".jpep("".$_SESSION['jpep']."")."<br>TAHUN ".$_SESSION['tahun']."</center></h5>";
...
SUM(F) bilf FROM analisis_mpsr amp, mpsr mp WHERE amp.kodsek=:kodsek AND amp.tahun=:tahun AND amp.jpep=:jpep AND amp.darjah=:ting  
...
if ($bilmp != 0)
{
	...
	while($rowmp = oci_fetch_array($smt))
	{
		...
		oci_bind_by_name($ressub,":kodmp",$kodmp);
		oci_execute($ressub);
...
<input type="button" name="export" value="EXPORT KE EXCEL" onclick="open_window('data-analisis-mptingsr-excel.php?ting=<?php echo $m;?>','win1');" />
</form>
