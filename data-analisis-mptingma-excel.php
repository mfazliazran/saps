...
SUM(Cp) bilcp, SUM(C) bilc, SUM(D) bild, SUM(E) bile, SUM(G) bilg FROM analisis_mpma amp, mpsmkc mp WHERE amp.kodsek=:kodsek  
and amp.tahun=:tahun AND amp.jpep=:jpep AND  amp.ting=:ting AND amp.kodmp=mp.kod Group BY amp.kodmp, mp.susun, mp.mp ORDER BY mp.susun";
...
function subjek_tak_ambil($kodmp)
{
 ...
 oci_bind_by_name($res,":kodmp",$kodmp);
 oci_execute($res);
