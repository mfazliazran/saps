...
$q_nting ="SELECT sr.keputusan, sr.gpc, sr.peratus, tm.nokp FROM tnilai_sr sr, tmuridsr tm WHERE sr.tahun='$tahun' AND sr.jpep='$jpep' AND sr.darjah='$ting' AND sr.kodsek='$kodsek' AND sr.nokp=tm.nokp and kodsek_semasa='$kodsek' GROUP BY sr.keputusan, sr.gpc, sr.peratus, tm.nokp ORDER BY sr.keputusan Desc, sr.gpc Asc, sr.peratus Desc";
// echo $q_nting;
$qrt = oci_parse($conn_sispa,$q_nting);
oci_execute($qrt);
$bildarjah=0;
while($datax = oci_fetch_array($qrt)){
	$bildarjah++;
}
...
//oci_execute($stmt);
$bilmurid=count_row("SELECT * FROM tmuridsr WHERE tahun$gting='$tahun' AND $gting='$ting' and KELAS$ting='$kelas' and kodsek_semasa='$kodsek' AND ($kodsekolah)");
