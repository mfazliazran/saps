...
$q_guru = OCIParse($conn_sispa,"SELECT * FROM tguru_kelas gk, tsekolah ts WHERE gk.kodsek='".$_SESSION['kodsek2']."' AND gk.tahun='".$_SESSION['tahun']."' AND gk.ting='$ting' AND gk.kelas='$kelas' AND gk.kodsek=ts.kodsek");
OCIExecute($q_guru);
OCIFetch($q_guru);
...
<!--><form action="ctk_misssr-jpn.php?ting=<?php echo $m;?>" method="POST" target="_blank"><!-->

<?php
//tnilai_sr
/*$datawujud = count_row("SELECT * FROM tnilai_sr WHERE tahun='".$_SESSION['tahun']."' AND jpep='".$_SESSION['jpep']."' AND darjah='$ting' AND kelas='$kelas' AND kodsek='$kodsek' ORDER BY keputusan Desc, gpc Asc, peratus Desc");
if($datawujud==0){
	$tabledata = "penilaian_muridsr";
}else{
	$tabledata = "tnilai_sr";
}*/
...
and mkh.darjah=sr.darjah and mkh.kelas=sr.kelas and mkh.jpep=sr.jpep AND (BILB > 0) AND NVL(BILC,0)=0 AND NVL(BILD,0)=0 AND NVL(BILE,0)=0 AND NVL(BILTH,0)=0 AND mkh.tahun='$tahun' AND mkh.darjah='$ting' AND mkh.kelas='$kelas' AND mkh.jpep='$jpep' ORDER BY sr.keputusan DESC, sr.gpc ASC, sr.peratus DESC";
...
$q_sub = oci_parse($conn_sispa,"SELECT * FROM mpsr mp, sub_guru sg WHERE sg.kodsek='$kodsek' and sg.tahun='$tahun' AND sg.ting='$ting' AND sg.kelas='$kelas' AND mp.kod=sg.kodmp ORDER BY mp.susun");
...
//echo "<h3><center>SEKURANG-KURANGNYA SATU B</center></h3>";
if($gurukel==1){
