...
<p class="subHeader">Analisis Headcount (BPTV)</p>
<?php
$datahc = $_GET['datahc'];
...
$qting = "SELECT DISTINCT hmp FROM $theadcount WHERE tahun='$tahun' AND kodsek='$kodsek' AND $tingdar='$ting' ORDER BY hmp";
...
while ($recordting = oci_fetch_array($rting)){
	...
	$q_atr1 = "SELECT $gmp FROM $tmarkah WHERE tahun='$tahunatr1' AND kodsek='$kodsek' AND $tahap='$tingatr1' AND jpep='$jpepatr1' AND $gmp IS NOT NULL";
	//if($kodsek=='BEA8638')
		//echo $q_atr1."<br>";
	...
	//if($kodsek=='BEA8638')
		//echo $calon_atr1."<br>";
	$q_atr2 = "SELECT $gmp FROM $tmarkah WHERE tahun='$tahunatr2' AND kodsek='$kodsek' AND $tahap='$tingatr2' AND jpep='$jpepatr2' AND $gmp IS NOT NULL";
	$qry_atr2 = oci_parse($conn_sispa,$q_atr2);
	oci_execute($qry_atr2);
	$calon_atr2 = count_row($q_atr2);
	$q_atr3 ="SELECT $gmp FROM $tmarkah WHERE tahun='$tahunatr3' AND kodsek='$kodsek' AND $tahap='$tingatr3' AND jpep='$jpepatr3' AND $gmp IS NOT NULL";
	$qry_atr3 = oci_parse($conn_sispa,$q_atr3);
	oci_execute($qry_atr3);
	$calon_atr3 = count_row($q_atr3);
	
	$qhc = "SELECT * FROM $theadcount WHERE tahun='$tahun' AND kodsek='$kodsek' AND hmp='$hmp' AND $tahap='$ting'";
	//echo $qhc."<br>";
	...
	switch ($ting) {
		case "P": case "p": case "T1": case "t1": case "T2": case "t2": case "T3": case "t3": case "D1": case "d1": case "D2": case "d2": case "D3": case "d3": case "D4": case "d4": case "D5": case "d5": case "D6": case "d6":
		{
			...
			$bil=$bil+1;
				...
				while($data_hc = oci_fetch_array($res_hc)){
					...
					$q_mp = "SELECT amp.kodmp, SUM(bcalon) bcalon, SUM(ambil) ambil, SUM(TH) th, SUM(A) bila, SUM(B) bilb, SUM(C) bilc, SUM(D) bild, SUM(E) bile
							 FROM $analisis WHERE amp.tahun='$tahun' AND amp.kodsek='$kodsek' AND amp.jpep='$jenpep' AND  amp.$tingdar='$ting' AND amp.kodmp=mp.kod and amp.kodmp='$hmp' and amp.kodmp not in ($minus) Group BY mp.susun,amp.kodmp";
					//if($kodsek='BEA8638')
						//echo $q_mp."<br>";
					//echo "<br>".$q_mp."<br>";
			...
			echo "    <td><div align=\"left\"><a href=hc-mp-kelas-bptv.php?datahc=".$kodsek."/".$tahun."/".$ting."/".$hmp.">$tempmp</div></td>\n";
		...
		{	
			...
			echo "  <tr>\n";
			echo "    <td><div align=\"center\">$bil</div></td>\n";
			echo "    <td><div align=\"left\"><a href=hc-mp-kelas-bptv.php?datahc=".$kodsek."/".$tahun."/".$ting."/".$hmp.">$hmp</div></td>\n";
			echo "    <td><div align=\"center\">".($bilcalon-$tovTH)."</div></td>\n";
