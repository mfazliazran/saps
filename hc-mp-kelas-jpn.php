...
while ($recordting=oci_fetch_array($rting)){
	...
	$gmp = "G$mp";
		...
		$q_atr1 = "SELECT $gmp FROM $tmarkah WHERE tahun='$tahunatr1' AND kodsek='$kodsek' AND $tahap='$tingatr1' AND kelas='$kelas' AND jpep='$jpepatr1' AND $gmp!=''";
	$qry_atr1 = oci_parse($conn_sispa,$q_atr1);
	oci_execute($qry_atr1);
	$calon_atr1 = count_row($q_atr1);
	$q_atr2 = "SELECT $gmp FROM $tmarkah WHERE tahun='$tahunatr2' AND kodsek='$kodsek' AND $tahap='$tingatr2' AND kelas='$kelas' AND jpep='$jpepatr2' AND $gmp!=''";
	$qry_atr2 = oci_parse($conn_sispa,$q_atr2);
	oci_execute($qry_atr2);
	$calon_atr2 = count_row($q_atr2);
	$q_atr3 = "SELECT $gmp FROM $tmarkah WHERE tahun='$tahunatr3' AND kodsek='$kodsek' AND $tahap='$tingatr3' AND kelas='$kelas' AND jpep='$jpepatr3' AND $gmp!=''";
	...
	switch ($ting) {
		case "P": case "p": case "T1": case "t1": case "T2": case "t2": case "T3": case "t3": case "D1": case "d1": case "D2": case "d2": case "D3": case "d3": case "D4": case "d4": case "D5": case "d5": case "D6": case "d6":
		{
			...
			//echo $ting."<br>";
				...
				while($data_hc = oci_fetch_array($res_hc)){
					...
					//$q_mp = "SELECT amp.kodmp, SUM(bcalon) bcalon, SUM(ambil) ambil, SUM(TH) th, SUM(A) bila, SUM(B) bilb, SUM(C) bilc, SUM(D) bild, SUM(E) bile							 FROM $analisis WHERE amp.tahun='$tahun' AND amp.kodsek='$kodsek' AND amp.jpep='$jenpep' AND  amp.$tingdar='$ting' AND amp.kelas='$kelas' AND amp.kodmp=mp.kod and amp.kodmp='$mp' and amp.kodmp not in ($minus) Group BY mp.susun,amp.kodmp";
					$q_mp = "SELECT amp.kodmp, SUM(bcalon) bcalon, SUM(ambil) ambil, SUM(TH) th, SUM(A) bila, SUM(B) bilb, SUM(C) bilc, SUM(D) bild, SUM(E) bile FROM $analisis WHERE amp.kodsek='$kodsek' and amp.tahun='$tahun' AND amp.jpep='$jenpep' AND  amp.$tingdar='$ting' AND amp.kelas='$kelas' AND amp.kodmp=mp.kod and amp.kodmp='$mp' and amp.kodmp not in ($minus) Group BY mp.susun,amp.kodmp";
					//if($kodsek=='BBA4042')		 
						//echo "<br>".$q_mp."<br>";
			...
			//echo "<td><div align=\"left\"><a href=hc-mp-ind.php?datahc=".$kodsek."/".$_SESSION['tahun']."/".$ting."/".$mp."/".$kelas.">$kelas</div></td>\n";
			echo "<td><div align=\"left\"><a href=hc-mp-ind-jpn.php?datahc=".$ting."|".$mp."|".$kelas.">$kelas</div></td>\n";
		...
		{	
			...
			echo "  <tr bgcolor='$bcol'>\n";
			echo "    <td><div align=\"center\">$bil</div></td>\n";
			//echo "    <td><div align=\"left\"><a href=hc-mp-ind.php?datahc=".$kodsek."/".$_SESSION['tahun']."/".$ting."/".$mp."/".$kelas.">$ting $kelas</div></td>\n";
			echo "<td><div align=\"left\"><a href=hc-mp-ind-jpn.php?datahc=".$ting."|".$mp."|".$kelas.">$kelas</div></td>\n";
			echo "    <td><div align=\"center\">".($bilcalon-$tovTH)."</div></td>\n";
