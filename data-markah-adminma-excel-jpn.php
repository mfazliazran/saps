...
$m="$ting&&kelas=$kelas&&namaguru=$namagu&&tingkatan=$tingkatan&&namasekolah=$namasek";
   ...
   header('Content-Disposition: attachment; filename="lembaranmarkahma.xls"');
...
oci_execute($q_mkdt);
$bilmkdt = count_row("SELECT * FROM markah_pelajar mkh, tnilai_sma ma WHERE mkh.nokp=ma.nokp AND mkh.tahun=ma.tahun and mkh.kodsek=ma.kodsek and mkh.ting=ma.ting and mkh.kelas=ma.kelas and mkh.jpep=ma.jpep  AND mkh.tahun='$tahun' AND mkh.kodsek='$kodsek' AND mkh.ting='$ting' AND mkh.jpep='$jpep' ORDER BY ma.keputusan DESC, ma.gpc ASC, ma.peratus DESC");

$q_murid = oci_parse($conn_sispa,"SELECT * FROM markah_pelajar mkh, tnilai_sma ma WHERE mkh.nokp=ma.nokp AND mkh.tahun=ma.tahun and mkh.kodsek=ma.kodsek and mkh.ting=ma.ting and mkh.kelas=ma.kelas and mkh.jpep=ma.jpep AND mkh.tahun='$tahun' AND mkh.kodsek='$kodsek' AND mkh.ting='$ting' $kodkelas and mkh.jpep=ma.jpep AND mkh.jpep='$jpep' ORDER BY ma.keputusan DESC, ma.gpc ASC, ma.peratus DESC");
oci_execute($q_murid);
...
echo "<h5><center>$namasek<br>LEMBARAN MARKAH MURID $tingkatan<br>".jpep($jpep)." <br>TAHUN ".$tahun."</center></h5>";
